...
class RetargetingPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not context.scene.rsl_retargeting_bone_list:
            row = layout.row(align=True)
            row.scale_y = 1.2
            row.operator(retargeting.BuildBoneList.bl_idname, icon='CON_ARMATURE')
            self.draw_import_export(layout)
            return
        ...
        row.scale_y = 1.2
        row.operator(retargeting.BuildBoneList.bl_idname, text='Rebuild Bone List', icon='CON_ARMATURE')
        row = subrow.row(align=True)
        row.scale_y = 1.2
        row.alignment = 'RIGHT'
        ...
        row = layout.row(align=True)
        row.scale_y = 1.4
        row.operator(retargeting.RetargetAnimation.bl_idname, icon='ARMATURE_DATA')
